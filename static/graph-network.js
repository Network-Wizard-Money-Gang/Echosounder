  ...
  methods: {
    ...
    getDHCP_CIDRScan : function(cible) {
      ...
      axios({
        ...
        // on appel la fonction de création de graphs :
        this.createCytoDHCPGraph(response.data, cible);
      }).catch((error) => {
        // si la requête échoue :
        mitt.emitter.emit('echo_toast_scan_error', "erreur : " + error);
        console.log(error);
    ...
    createCytoVlanGraph : function(scan_data) {
      ...
      nodes.forEach(function(nodeI) {
        if(nodeI.data.id == scan_data.local_data.gateway_ip) {return}; // on évite de faire boucler la gateway avec elle-même
    ...
    // fonction de création du graph à partir d'un scan CIDR normé (par exemple ARP)
    ...
    // fonction de création du graph à partir d'un scan DHCP normé (par exemple DHCP)
    createCytoDHCPGraph : function(scan_data, cidr) {
      if(scan_data.scan.length == 0) { // on vérifie qu'on a pas juste un scan vide
        mitt.emitter.emit('echo_toast_scan_error', "erreur : Scan reçu vide" );
        return;
      }
      ...
      // ajout de l'entité serveur DHCP :
      if(scan_data.scan.hasOwnProperty('DHCP_server')) {
        nodes.push(
          {
            group : 'nodes', 
            data: {
              id : scan_data.scan.DHCP_server.ip,
              label : scan_data.scan.DHCP_server.ip,
              type : 'IP',
              typeip : ipaddr.parse(scan_data.scan.DHCP_server.ip).range(),
              data : scan_data.scan.DHCP_server.data,
              data_ip : scan_data.scan.DHCP_server.ip,
              parent : cidr,
            }
          }
        )
      };
      // on ajoute la liste des IP secondaires
      scan_data.scan.IPs.forEach(function(nodeAdd) {
        nodes.push(
          {
            group:'nodes',
            data: {
              ...
              label : (nodeAdd),
              type : 'IP',
              typeip : ipaddr.parse(nodeAdd).range(),
              data : {},
              data_ip : nodeAdd,
              parent : cidr,
            },
